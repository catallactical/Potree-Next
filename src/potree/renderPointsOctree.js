...
function getOctreeState(renderer, octree, attributeName, flags = []) {
	...
	const key_mappings = octree.material.mappings.map(
		(m) => m.name + "_" + m.inputs.join("_"),
	);
	const key = `${octree.state_id}_${flags.join(";")}_${key_mappings}`;

	const state = octreeStates.get(key);
	...
	if (state === undefined) {
		...
		const nodesBuffer = new ArrayBuffer(10_000 * WGSL_NODE_BYTESIZE);
		const nodesGpuBuffer = renderer.createBuffer({
			size: nodesBuffer.byteLength,
			usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
		});

		const attributesDescBuffer = new ArrayBuffer(16_384);
		const attributesDescGpuBuffer = renderer.createBuffer({
			size: attributesDescBuffer.byteLength,
			usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
		});

		const colormapBuffer = new ArrayBuffer(4 * 256);
		const colormapGpuBuffer = renderer.createBuffer({
...
function updateUniforms(octree, octreeState, drawstate, flags) {
	...
	const uniformsView = new DataView(data);

	{
		...
		const isAdditive = !(octree.loader.constructor.name === "Potree3Loader");
		uniformsView.setUint32(284, isAdditive ? 1 : 0, true);

		const bb = octree.getBoundingBoxWorld();
		...
		const att = octree.attributes.attributes.find(
			(a) => a.name === "classification",
		);
		let classificationAttIndex = octree.attributes.attributes.indexOf(att);
		classificationAttIndex = Number.isInteger(classificationAttIndex)
			? classificationAttIndex
			: -1;
...

	{
		...
		const attributeView = new DataView(attributesDescBuffer);

		const set = (index, args) => {
			const clampBool = args?.settings?.clamp ?? false;
			const clamp = clampBool ? 1 : 0;

			const byteSize = args.attribute?.byteSize ?? 0;
			const dataType = args.attribute?.type?.ordinal ?? 0;
			const numElements = args.attribute?.numElements ?? 1;

			const range_min = [0, 0, 0, 0];
			const range_max = [1, 1, 1, 1];
			...
			// let attributeName = Potree.settings.attribute;
			const attributeName = args.settings.name;
			const material = octree.material;
			const mapping = material.selectedMappings.get(attributeName);
		...
		let i = 0;
		for (const [attributeName, settings] of octree.material.attributes) {
...
function updateNodesBuffer(
	...
	for (const [i, node] of nodes) {
		...
		let childmask = 0;
		for (let j = 0; j < 8; j++) {
			const visible = node.children[j]?.visible ?? false;
		...
		// debugger;
		const nodeSpacing = octree.spacing / 2 ** node.level;
		...
		view.setFloat32(
			WGSL_NODE_BYTESIZE * i + 12,
			bbWorld.min.y + bb.min.y,
			true,
		);
		view.setFloat32(
			WGSL_NODE_BYTESIZE * i + 16,
			bbWorld.min.z + bb.min.z,
			true,
		);
		view.setFloat32(
			WGSL_NODE_BYTESIZE * i + 20,
			bbWorld.min.x + bb.max.x,
			true,
		);
		view.setFloat32(
			WGSL_NODE_BYTESIZE * i + 24,
			bbWorld.min.y + bb.max.y,
			true,
		);
		view.setFloat32(
			WGSL_NODE_BYTESIZE * i + 28,
			bbWorld.min.z + bb.max.z,
			true,
		);
		...
		view.setUint32(
			WGSL_NODE_BYTESIZE * i + 48,
			node.geometry.numVoxels > 0 ? 1 : 0,
			true,
		);
	...
	renderer.device.queue.writeBuffer(
		...
		nodesBuffer,
		0,
		WGSL_NODE_BYTESIZE * octree.visibleNodes.length,
	);
}
...
async function renderOctree(octree, drawstate, flags) {
	...
	const octreeState_points = getOctreeState(renderer, octree, attributeName, [
		...flags,
		"SPLAT_TYPE_0",
	]);
	const octreeState_quads = getOctreeState(renderer, octree, attributeName, [
		...flags,
		"SPLAT_TYPE_1",
	]);
	...
	if (mapping) {
		...
		for (const state of [octreeState_quads, octreeState_points]) {
			if (listing) {
				const { colormapBuffer, colormapGpuBuffer } = state;

				const u8 = new Uint8Array(colormapBuffer);
				const defaultValue = listing?.DEFAULT ?? 0;
				for (let i = 0; i < 256; i++) {
				...
				for (const index of Object.keys(listing)) {
					if (index === "DEFAULT") {
						continue;
					}

					const value = listing[index];
...

	{
		...
		const { pipeline, uniformBindGroup, nodesBindGroup } = octreeState_points;
		...
		for (const [index, node] of smallNodes) {
			const numElements = node.geometry.numElements;

			const bufferBindGroup = getCachedBufferBindGroup(
				renderer,
				pipeline,
				node,
			);
			...
			if (node.dirty) {
				const gpuBuffer = renderer.getGpuBuffer(node.geometry.buffer);
				renderer.device.queue.writeBuffer(
					gpuBuffer,
					0,
					node.geometry.buffer,
					0,
					node.geometry.buffer.byteLength,
				);
	...
	if (octree.isHighlighted) {
		const box = octree.boundingBox.clone().applyMatrix4(octree.world);
		const position = box.min.clone();
		position.add(box.max).multiplyScalar(0.5);
		const size = box.size();
		const color = new Vector3(255, 255, 0);
